...
export const updateHome = async (req, res) => {
  try {
    ...
    if (home.banner_img) {
      // delete old image before uploading a new image
      await cloudinary.v2.uploader.destroy(user.home.banner_img.public_id);

      const myCloud = await cloudinary.v2.uploader.upload(home.banner_img, {
        folder: "portfolio",
      });
...
export const updateSkillImages = async (req, res) => {
  try {
    ...
    if (skillsCubeImg.image1) {
      await cloudinary.v2.uploader.destroy(user.skillsCubeImg.image1.public_id);
    ...
    if (skillsCubeImg.image2) {
      await cloudinary.v2.uploader.destroy(user.skillsCubeImg.image2.public_id);
    ...
    if (skillsCubeImg.image3) {
      await cloudinary.v2.uploader.destroy(user.skillsCubeImg.image3.public_id);
    ...
    if (skillsCubeImg.image4) {
      await cloudinary.v2.uploader.destroy(user.skillsCubeImg.image4.public_id);
    ...
    if (skillsCubeImg.image5) {
      await cloudinary.v2.uploader.destroy(user.skillsCubeImg.image5.public_id);
    ...
    if (skillsCubeImg.image6) {
      await cloudinary.v2.uploader.destroy(user.skillsCubeImg.image6.public_id);

...
import { addBackendProject, getUser } from "../../../redux/actions/User";
...
const BackendProject = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    await dispatch(
      addBackendProject(title, techstack, image, gitLink, demoLink)
    );
    dispatch(getUser());
    setButtonText("Added");
    ...
    <section className="contact login adminpanelcontainer" id="connect">
      ...
      <div className="adminpanel-form">
        <div className="admin-container-inputbox">
          <ImageBox label="Picture" value={image} onChange={handleImage} />
      ...
      <div className="add-project-details">
        {user?.backendProjects?.map((item) => (
          <ProjectCard key={item._id} item={item} i={3} />
        ))}
      </div>
    </section>
  );

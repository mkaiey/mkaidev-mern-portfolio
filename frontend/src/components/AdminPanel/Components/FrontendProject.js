...
const FrontendProject = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    await dispatch(
      addFrontendProject(title, techstack, image, gitLink, demoLink)
    );
    dispatch(getUser());
    setButtonText("Added");
  ...
  return (
    <section className="contact login adminpanelcontainer" id="connect">
      <h2>Manage Frontend Projects</h2>
      ...
      <div className="add-project-details">
        {user?.frontendProjects?.map((item) => (
          <ProjectCard key={item._id} item={item} i={1} />
        ))}
      </div>
    </section>
  );
